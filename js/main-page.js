...
function editTable(tableid, editbtn, editable, tableideditable, array, numbered){
...
function removeRow(rowno, tableideditable, numbered){
	...
	for(var i = rowno; i < rows;i++){
		...
		document.getElementById(tableideditable).rows[i].cells[0].innerHTML = (i-1).toString();
		document.getElementById(tableideditable).rows[i].deleteCell(tempcol-1);
		document.getElementById(tableideditable).rows[i].innerHTML += '<td style="width:10px;"><button class="btn btn-xs" onclick="removeRow('+i+', '+"'"+tableideditable+"', "+numbered+')"><icon class="fa fa-times"></button><button id="1" class="btn btn-xs" onclick="addRow('+i+', '+"'"+tableideditable+"', "+numbered+')"><icon class="fa fa-plus"></button></th>';
	}
...
function addRow(rowno, tableideditable, numbered){
	...
	for(var j = numbered; j < columns;j++){
			...
			'<input class="form-control" type="text" id="input-'+i+'-1">'
		}
