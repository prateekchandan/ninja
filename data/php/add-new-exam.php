...
$examno=$_POST[$type."-new-exam-name"];
$check=mysqli_query($con,"select * from college_entrance_test where cid=".$cid." && name=".$examno." && type='".$type."'");
...
$catexam=mysqli_fetch_array(mysqli_query($con,"select category from exam where eid=".$examno))['category'];
$catexam=json_decode($catexam,true);
$query="insert into `college_entrance_test` (`cid`,`type`,`name`";
			$append=" values (".$cid.",'".$type."',".$examno;
        foreach ($categories as $key) {
            $append.=",";
            $query.=",";
            $query.=$key['id'];
